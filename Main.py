...
def barchart():
    ...
    fig = px.bar(df_selection, y='AnnualSalary', x='JobTitle', text_auto='.2s', title="Controlled text sizes, positions and angles",
                     color='JobTitle',
                     color_discrete_sequence=px.colors.qualitative.Dark24)  # Use a predefined color sequence
    fig.update_traces(textfont_size=18, textangle=0, textposition="outside", cliponaxis=False)
    st.plotly_chart(fig, use_container_width=True, theme="streamlit")

# Scatter plot for Annual Salary over Hire Date
def scatter_plot():
    theme_plotly = None # None or streamlit
    with div2:
...
# 100% Stacked bar chart for gender distribution by department
...
if selected == "Home":
    metrics()
    pie()
    donut_chart()  # Add donut chart to the Home section
    barchart()
    scatter_plot()  # Add scatter plot to the Home section
    stacked_barchart()  # Add 100% stacked bar chart to the Home section
